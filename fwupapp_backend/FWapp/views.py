...
class FirmwareCUDView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            data['fw_package_url'] = file_url
        except Exception as e:
            logger.error(f"Error uploading file to Azure: {e}")
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        serializer = FirmwarePackageSerializer(data=data, context={'request': request})
    ...
    def put(self, request):
        ...
        if 'fw_package' in request.FILES:
            try:
                file_url = upload_file_to_azure(request.FILES['fw_package'])
                data['fw_package_url'] = file_url
            except Exception as e:
                logger.error(f"Error uploading file to Azure: {e}")
                return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        serializer = FirmwarePackageSerializer(firmware, data=data, partial=True, context={'request': request})
        if serializer.is_valid():
            firmware = serializer.save()
            return Response(FirmwarePackageSerializer(firmware, context={'request': request}).data, status=status.HTTP_200_OK)
